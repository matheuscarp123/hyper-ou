  ...
  return (
    <main className="flex min-h-screen flex-col items-center p-4 pt-8 relative">
      ...
      >
        ...
        {/* Ad Banner */}
        <AdBanner id="physique-banner" className="mb-6" />
            ...
            {filteredPhysiques.map((physique, index) => {
              if (physique.premium) {
                return (
                  <PremiumGate
                    key={physique.id}
                    feature={`${physique.id}Plan` as any}
                    title={`Plano ${physique.name}`}
                    description="Acesse os melhores planos de treino dos maiores nomes do fisiculturismo"
                    allowRewardedUnlock={true}
                  >
                    <PhysiqueCard
                      physique={physique}
